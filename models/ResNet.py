...
class Model(nn.Module):
    def __init__(self, config, resnet_type='18', nclasses=2):
        self.in_planes = 16
        ...
        self.conv5 = nn.Conv2d(512 * block.expansion, 256, kernel_size=(config["num_nodes"], 3), stride=(1, 1), padding=(0, 1),
                               bias=False)
        ...
        self.fc = nn.Linear(256 * 2, config["enc_dim"])
        self.fc_mu = nn.Linear(config["enc_dim"], nclasses) if nclasses >= 2 else nn.Linear(config["enc_dim"], 1)
